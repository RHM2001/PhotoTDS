...
public class CommentPhoto {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CommentPhoto window = new CommentPhoto(null);
	 ...
	 */
	public CommentPhoto(AddPublicacion frmPhoto) {
		frmFoto = frmPhoto;
		initialize();
	}
	...
	private void initialize() {
		...
		// inserto imagen en El icon del panel (se puede hacer un variable de tipo
		// String y llamarle Imagen,
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				Controlador.getUnicaInstancia().registrarFoto(
						Controlador.getUnicaInstancia().getNombreUserUsuarioActual(), cadenaToday, textArea.getText(),
						hashtags, frmFoto.getFotos().get(0));

				System.out.println("Fotos del usuario : ");

				try {
					LinkedList<Publicacion> lista = (LinkedList<Publicacion>) Controlador.getUnicaInstancia()
							.recuperarPublicacionesUsuario(
									Controlador.getUnicaInstancia().getNombreUserUsuarioActual());
	...
	private void muestraHome() throws ParseException, IOException {
		if (frmHome == null)
			frmHome = new Home(null, this, null);
